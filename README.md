...
`/api/ember/hourly-scan` is the first backend scan shape. `POST` with a `state` payload can dry-run Ember detection for one workspace. `GET` is wired in `vercel.json` as an hourly cron and scans recent `apex_user_state` rows, then writes `apex_ember_states`, `apex_ember_messages`, and `apex_ember_notification_events` through the Supabase REST API. Production persisted scans require `SUPABASE_SERVICE_ROLE_KEY` plus `EMBER_SCAN_SECRET` or `CRON_SECRET`; keep those server-only in Vercel and never expose the service role key to the browser.

`/api/ember/morning-brief` uses the same scan engine with a `morning` event key and a 26-hour guardrail so the daily briefing does not repeat. It is scheduled in `vercel.json` at `12:30 UTC`, which currently approximates a morning Central-time brief; once user profile timezones are stored, the scanner should filter users into their own 7-9 AM local window.
